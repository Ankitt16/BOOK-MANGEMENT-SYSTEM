...
def userLogin(request):
    ...
    if request.method == 'POST':
        ...
        if user:
            ...
            return HttpResponseRedirect('/BRMapp/view-books.html')
        else:
...
def searchBook(request):
    form=SearchForm()
    res=render(request,'BRMapp/search-book.html',{'form':form})
    return res
@login_required(login_url="/BRMapp/login/")
def search(request):
...
def editBook(request):
    ...
    res=render(request,'BRMapp/edit-book.html',{'form':form,'book':book})
    return res
@login_required(login_url="/BRMapp/login/")
def edit(request):
    if request.method=='POST':
...
def viewBooks(request):
    ...
    res=render(request,'BRMapp/view-book.html',{'books':books,'username':username})
    return res
